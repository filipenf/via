...
The default interactive pane is id `agent`. Bus messages with no recipient default to `orchestrator` once spawned;
`via agent send` errors if the recipient is not registered.
...
PTY orchestration loops can use `--wait` instead of `sleep` + immediate `inbox`: the command returns as soon as a
message lands, or after the timeout with an empty result.
...
Tasks survive via restarts; they live on a **board** within a **workspace** (sanitized absolute project `cwd`), not in
the ephemeral instance directory. Restart / `via task list` reuses the existing active board; only create a board when
none exist yet, or explicitly with `via task board new --id …`.
...
via task claim task-1   # auto-appends "Claimed by …" (via inserts ---)
via task update task-1 --append $'### coder\n- Done: reproduced failure\n- Next: implement fix'
via task review task-1  # auto-appends handoff-to-human note
via task done task-1    # auto-appends done note
```

### Lifecycle checklist

1. **Create** — short title + rich `-m` body (strongly expected; CLI warns if omitted).
2. **Claim** — `via task claim <id>` (sets assignee to you, status `in_progress`; via
   auto-appends a claim note).
3. **Work** — keep status `in_progress`; **append status updates to the body** as you make
   progress (see below) so other agents and the human share context.
4. **Review** — `via task review <id>` or `via task update <id> --status review` (notifies
   the primary `agent` pane and opens the review surface). `via task review` auto-appends
   a handoff note; still add your own summary of what changed and how to verify.
5. **Done** — only after human/reviewer sign-off: `via task done <id>` (auto-appends a done
   note).
...
The task body is the shared log for humans and other agents.

**Automatic (via):** `claim`, `via agent assign --task`, `via task review`, and
`via task done` append a short `### via` note. You do not need to restate those.

**Manual (you):** use `--append` for progress, blockers, decisions, and review prep.
Send only the note text — via inserts the `---` separator before each append onto
an existing body (do not prefix `---` yourself):

```bash
via task update <id> --append $'### 2026-07-22 coder\n- Done: reproduced failure.\n- Next: implement fix.\n'
```

`-m/--body` still **replaces** the whole body; use it only when rewriting the goal/scope.
`--append`, `--body`, and `--clear-body` are mutually exclusive.
...
- **Repro / acceptance**: how to verify (commands, expected output, test names that must pass).
- **Dependencies**: link upstream tasks with `--blocked-by <ids>` (if any)
- **Pointers**: `file_path:line` references for the code that motivates the task, and any prior-session notes
  (`@skills/...`, Obsidian notes, etc.).

Linking in the body: use the `via:<id>` form (the same prefix the board rows use) so dependencies are greppable in
`via task list` / `via task show`.
...
`via task` works from a via agent pane (uses instance `cwd`) or from the project directory without a live instance.

When a live via instance is running, task transitions also deliver to agents:

- **Unassigned create**: notifies the primary `agent` pane (and `orchestrator` if spawned) that work is available on the
  board
- **Assignee**: notified on create, assignee change, or status change (ACP agents get a live prompt; PTY agents read
  `via agent inbox`)
- **Review gate**: moving a task to `review` notifies the primary `agent` pane for human sign-off **and** opens the
...
When picking up a task, read `via task show <id>` first — prior **Status updates** are the handoff context.
...
| `via task list\|create\|show\|path\|claim\|update\|done`      | Tasks on the active board (`update --append` for status notes) |

## Sandbox notes

These commands use local Unix sockets and files under the **instance** runtime directory (`instances/<pid>/`) for the
agent bus; tasks use `workspaces/` under the via data directory.
